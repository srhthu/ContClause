...
CUDA_VISIBLE_DEVICES=1,2 accelerate launch --num_processes 2 --main_process_port 5677 -m pretrain.demo_train_ds \
--model_path $model_path --dtype bf16 \
--max_steps 200 --device_bs ${device_bs} --grad_acc_steps ${gaccstep} --log_steps 5 \
--lib accelerate --ds_config pretrain/ds_config/zero1_fp16.json --lr 1e-5
done
done
